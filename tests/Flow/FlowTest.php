...
class FlowTest extends AbstractFlowTest
{
    ...
    public function testJob(DriverInterface $driver, IpStrategyInterface $ipStrategy, Closure $job, int $resultNumber, ?Throwable $resultException): void
    {
        ...
        $flow = new Flow($job, $ipStrategy, $driver);
        $flow->pipe(function (Ip $ip, ?Throwable $exception) use ($driver, $resultNumber, $resultException) {
            $driver->stop();
    ...
    public function testJobs(DriverInterface $driver): void
    {
        ...
        $flow = new Flow($jobs, null, $driver);

        $ips = [];
        $flow->pipe(function (Ip $ip) use ($driver, &$ips, $ip1, $ip2) {
            $ips[] = $ip;
            if(count($ips) === 2) {
